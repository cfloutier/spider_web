...
void setup() 
{
    ...
    spiralWeb =  new DrawingGenerator();
    data = new DrawingData();
    dataGui = new DataGUI();
    
    spiralWeb.center =  new PVector(800,400);
  ...
  background(0);
   ...
   if (record) 
   {
    // Note that #### will be replaced with the frame number. Fancy!
      ...
      fileName = "Export/Web_" + year() + "-" + month() + "-" + day() + "_" + hour() + "-" + minute() + "-" + second(); 
      if (mode == 0)
    ...
    spiralWeb.center =  new PVector(width/2,height/2);
